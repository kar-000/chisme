...
def channel(client, headers, server_id):
    resp = client.post(f"/api/servers/{server_id}/channels", json={"name": "msg-room"}, headers=headers)
...
class TestSendMessage:
    ...
    def test_send_to_nonexistent_channel(self, client: TestClient, headers, server_id):
        resp = client.post(
            f"/api/servers/{server_id}/channels/9999/messages",
            json={"content": "Hi"},
            headers=headers,
        )
...
class TestGetMessages:
    ...
    def test_pagination(self, client: TestClient, headers, channel):
        ...
        resp = client.get(
            f"/api/servers/{channel['server_id']}/channels/{channel['id']}/messages?limit=2&offset=0",
            headers=headers,
        )
        assert resp.status_code == 200
